...
import { Shield, Lock, User, ShieldAlert, Cpu, ArrowRight } from 'lucide-react';
...
const AuthPage: React.FC<AuthPageProps> = ({ onLogin }) => {
  ...
  return (
    <div className="h-screen w-screen bg-[#05070a] flex items-center justify-center p-6 overflow-hidden relative">
      {/* Background Decor */}
      ...
      <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 z-10">
        ...
        <div 
          ...
          className="group relative bg-slate-900/40 border border-white/5 hover:border-emerald-500/40 p-10 rounded-[40px] cursor-pointer transition-all hover:scale-[1.02] hover:shadow-2xl hover:shadow-emerald-500/10"
        >
          <div className="mb-8 p-4 bg-emerald-500/10 rounded-2xl w-fit border border-emerald-500/20">
            <User className="w-8 h-8 text-emerald-500" />
          </div>
          <h2 className="text-2xl font-black text-white uppercase tracking-tight mb-2">Restricted Operator</h2>
        ...
        {/* Admin Role Card */}
        <div 
          className={`relative bg-slate-900/40 border border-white/5 p-10 rounded-[40px] transition-all overflow-hidden ${
        ...
        >
          {!showAdminLogin ? (
            <>
              ...
              <h2 className="text-2xl font-black text-white uppercase tracking-tight mb-2">System Administrator</h2>
              <p className="text-slate-500 text-sm leading-relaxed mb-8">
            ...
            <form onSubmit={handleAdminLogin} className="h-full flex flex-col animate-in fade-in zoom-in-95 duration-300">
              <div className="flex items-center justify-between mb-8">
                ...
                <button 
                  ...
                  className="text-[10px] font-black text-slate-600 hover:text-white uppercase tracking-widest"
                >
                  Cancel
                </button>
              </div>
              <h2 className="text-xl font-black text-white uppercase mb-6">Enter Admin Passcode</h2>
              <input 
                autoFocus
                type="password"
                placeholder="****"
                ...
                className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-6 text-2xl tracking-[1em] text-center font-mono text-white focus:outline-none focus:border-red-500/50 transition-all mb-4"
              />
              {error && <p className="text-red-500 text-[10px] font-black uppercase text-center mb-4">{error}</p>}
              <button 
                type="submit"
                className="mt-auto w-full py-4 bg-red-600 hover:bg-red-500 text-white font-black uppercase tracking-widest rounded-2xl transition-all"
              >
                Authorize Root
              </button>
              <p className="mt-4 text-center text-[9px] text-slate-600 font-mono italic">MOCK_PASSCODE: 7777</p>
            </form>
          )}
        </div>
      ...
      <div className="absolute bottom-10 flex flex-col items-center gap-4">
        <div className="flex items-center gap-3 opacity-30">
          ...
          <span className="text-sm font-black text-white uppercase tracking-[0.5em]">Sentinel IDS</span>
        </div>
